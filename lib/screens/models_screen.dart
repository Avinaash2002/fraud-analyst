      ...
      body: SafeArea(
        child: Consumer<AppProvider>(builder: (context, provider, _) {
          ...
          return SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (_activeTab == 1)
              Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Center(child: GestureDetector(
                onTap: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('PDF export coming soon!'), backgroundColor: Color(0xFF2A9D8F))),
                child: Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
                  ...
                  child: const Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.download, size: 18, color: Color(0xFF6B7280)), SizedBox(width: 8),
                    Text('Download PDF', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF1A1A2E))),
                  ])),
              ))),
      ...
      return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            _SmallMetric(label: 'Accuracy', value: '${(model.accuracy * 100).toStringAsFixed(1)}%'),
            _SmallMetric(label: 'Precision', value: '${(model.precision * 100).toStringAsFixed(1)}%'),
            _SmallMetric(label: 'Recall', value: '${(model.recall * 100).toStringAsFixed(1)}%'),
            _SmallMetric(label: 'F1', value: '${(model.f1Score * 100).toStringAsFixed(1)}%'),
          ]),
        ]),
      );
    }).toList()));
    ...
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Column(children: [
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...models.map((model) {
            ...
            return Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [
              ...
              const SizedBox(width: 10),
              SizedBox(width: 50, child: Text('${(value * 100).toStringAsFixed(1)}%', textAlign: TextAlign.end, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF1A1A2E)))),
            ]));
          }),
        ]),
      );
      ...
      if (bestModel != null) Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
            '${bestModel.modelName} achieves the highest overall performance with ${(bestModel.accuracy * 100).toStringAsFixed(1)}% accuracy, making it the recommended model for production deployment.',
